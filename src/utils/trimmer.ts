...
class TrimSanitizer {
    ...
    public sanitize(classInstance: Object) {
        ...
        trimData
            .filter((datum) => {
                return (
                    (classInstance as any)[datum.propertyName] !== null &&
                    (classInstance as any)[datum.propertyName] !== undefined
                );
            })
            .forEach((datum) => {
                if (datum.each) {
                    if (Array.isArray((classInstance as any)[datum.propertyName])) {
                        ((classInstance as any)[datum.propertyName] as any[]).forEach(
                            (item, index, array) => {
                                if (
                                    item !== null &&
                                    item !== undefined &&
                    ...
                    if (typeof (classInstance as any)[datum.propertyName] === 'string') {
                        (classInstance as any)[datum.propertyName] = (
                            (classInstance as any)[datum.propertyName] as string
                        ).trim();
                    }
                }
            });
